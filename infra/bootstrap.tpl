...
VALIDATOR_SECRET_DATA=$(gcloud secrets versions access latest --secret="${validator_secret_id}")
echo "$VALIDATOR_SECRET_DATA" > ~/go-tableland/docker/deployed/${TBLENV}/api/.env_validator

# Fetch the secret from Google Cloud Secret Manager
GRAFANA_SECRET_DATA=$(gcloud secrets versions access latest --secret="${grafana_secret_id}")
echo "$GRAFANA_SECRET_DATA" > ~/go-tableland/docker/deployed/${TBLENV}/grafana/.env_grafana

# Fetch the secret from Google Cloud Secret Manager
HEALTHBOT_SECRET_DATA=$(gcloud secrets versions access latest --secret="${healthbot_secret_id}")
echo "$HEALTHBOT_SECRET_DATA" > ~/go-tableland/docker/deployed/${TBLENV}/healthbot/.env_healthbot

# Fetch the grafana db from Google Cloud Storage
gsutil cp gs://grafana_db_bucket/grafana.db ~/go-tableland/docker/deployed/${TBLENV}/grafana/data/grafana.db
...
sudo su - validator -c 'cd ~/go-tableland/docker && make staging-up'